...
class MainActivity : AppCompatActivity(), CategoryAdapter.CategoryItemClickListener {
    ...
    private fun bindCategory(){
        val rvKategori = binding.rvKategori
        val category: List<Category> = listOf(
            Category(
                id = 1,
                name = "Transfer",
                description = "Kategori untuk transfer uang",
                image = R.drawable.baseline_attach_money_24
            ),
            Category(
                id = 2,
                name = "Give Tip",
                description = "Kategori untuk memberikan tips",
                image = R.drawable.baseline_money_24
            ),
            Category(
                id = 3,
                name = "Top Up",
                description = "Kategori untuk top up saldo",
    ...
    override fun onCategoryItemClicked(category: Category) {
        Log.d("MainActivity", "Category item clicked: ${category.name}")
        when (category.id) {
            1 -> {
